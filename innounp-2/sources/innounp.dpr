...
function CompareIntegerValues(const A, B: Integer): Integer;
begin
...
function CompareInt64Values(const A, B: Int64): Integer;
begin
  if A < B then Result := -1
  else if A > B then Result := 1
  else Result := 0;
...
begin
  ...
  Result := CompareIntegerValues(A^.Location^.FirstSlice, B^.Location^.FirstSlice);
  if Result <> 0 then Exit;
  Result := CompareInt64Values(A^.Location^.StartOffset, B^.Location^.StartOffset);
  if Result <> 0 then Exit;
  Result := CompareInt64Values(A^.Location^.ChunkSuboffset, B^.Location^.ChunkSuboffset);
  if Result <> 0 then Exit;
  Result := CompareIntegerValues(A^.FileNumber, B^.FileNumber);
end;
...
begin
  ...
  try
    ...
    if (length(FileMasks) > 0) or (length(BaseDirToStrip) > 0) then
